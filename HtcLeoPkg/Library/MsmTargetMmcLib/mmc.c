...
static inline UINT64 lldiv(UINT64 dividend, UINT32 divisor)
{
	...
	if (dividend >> 32 == 0) {
		__res = (UINT32)(dividend) % divisor;
		dividend = (UINT32)(dividend) / divisor;
	} else {
		UINT64 __res = dividend;
		UINT64 b = divisor;
		UINT64 d = 1;
		UINT32 high = __res >> 32;
		UINT64 res = 0;
		
		if (high >= divisor) {
			high /= divisor;
			res = (UINT64)(high) << 32;
			__res -= (UINT64)(high * divisor) << 32;
		}
...
int _mmc_read(mmc_t *mmc, UINT64 src, uchar *dst, int size)
{
	char *buffer;
	int i;
...
int mmc_switch(mmc_t *mmc, UINT8 set, UINT8 index, UINT8 value)
{
	mmc_cmd_t cmd;
...
int sd_switch(mmc_t *mmc, int mode, int group, UINT8 value, UINT8 *resp)
{
	mmc_cmd_t cmd;
...
retry_scr:
	...
	err = sd_switch(mmc, SD_SWITCH_SWITCH, 0, 1, (UINT8 *)&switch_status);
