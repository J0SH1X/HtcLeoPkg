...
#if DEBUG_I2C
static void dump_status(u_int32_t status)
{
...
{
	u_int32_t status	= readl(dev.pdata->i2c_base + I2C_STATUS);
	bool not_done = true;
...
int msm_i2c_read(int chip, u_int8_t reg, void *buf, size_t count)
{
	int rc = ERR_NOT_READY;
	if (!dev.pdata) {
		I2C_DBG(ALWAYS, "[MSM-I2C]: %s: called when driver is not installed\n", __func__);
